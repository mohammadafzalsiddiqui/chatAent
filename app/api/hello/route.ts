...
// 	temperature: 0.7,
// 	model: "gpt-4o", // You can change this to "gpt-3.5-turbo" or another OpenAI model
...
export async function POST(request: Request) {
	try {
		...
		// Create tools with specific swap capabilities
		const tools = createAptosTools(aptosAgent, {
			filter: [
				"aptos_balance", 
				"aptos_balance", 
				"aptos_get_token_detail",
				"PanoraSwapTool",  // Add Panora DEX swap
				"LiquidSwapSwapTool"  // Add LiquidSwap DEX swap
			]
		})
		
		const memory = new MemorySaver()

		// Create React agent with improved swap instructions
		const agent = createReactAgent({
			llm,
			tools,
			checkpointSaver: memory,
        ...
        For token swaps, you can use either:
        - PanoraSwapTool: Use this for swapping tokens through Panora (requires API key)
        - LiquidSwapSwapTool: Use this for swapping tokens through LiquidSwap

        When performing a swap, follow these steps:
        1. First check token balances using AptosBalanceTool
        2. Get token information using AptosGetTokenDetailTool
        3. Get token prices using AptosGetTokenPriceTool if available
        4. Execute the swap using either PanoraSwapTool or LiquidSwapSwapTool with parameters:
           - fromToken: the source token address
           - toToken: the destination token address
           - amount: amount in the source token's smallest unit, accounting for decimals
           - slippage: (optional) percentage as a string, e.g. "1.0" for 1%
